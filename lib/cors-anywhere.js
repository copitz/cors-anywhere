...
function withCORS(headers, request) {
  ...
  if (request.headers['access-control-request-headers']) {
    ...
    headers['access-control-allow-headers'] = 'X-Requested-With,X-Prototype-Version,Content-Type,Cache-Control,Pragma,Origin,Cookie';
  }

  headers['access-control-expose-headers'] = Object.keys(headers).join(',').replace(/(^|[-,])([a-z])/g, function (g) { return g.toUpperCase(); });
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (location.host === 'crossdomain.xml') {
      // Added Crossdomain.xml, TODO better...
      res.writeHead(200, {'Content-Type': 'text/xml'});
      res.end('<?xml version="1.0"?><!DOCTYPE cross-domain-policy SYSTEM "http://www.macromedia.com/xml/dtds/cross-domain-policy.dtd"><cross-domain-policy><allow-access-from domain="*" /></cross-domain-policy>');
      return;
    }
